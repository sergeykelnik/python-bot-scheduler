...
1. **`/schedule`** - начать создание
2. **Указать чат** - `me` (себе) или ID чата
3. **Ввести сообщение** - текст для отправки
4. **Выбрать расписание** - один из вариантов выше

## 📋 Управление:

- **`/list`** - посмотреть все расписания
- **`/manage`** - управлять (удалить/приостановить)
- **`/getchatid`** - узнать ID чата

---

*Просто выберите команду из меню внизу ⬇️*
        """
        self.bot.send_message(chat_id, text)
